...
- 详情浮层页脚钉住 + 取消恢复撤销条（2026-09-26，用户第二轮验收反馈）
  - **浮层**：原先头部、键值、快速操作、最近记录、编辑/删除全塞在同一个 `verticalScroll` 里，所以按钮位置随内容条数漂移，内容一多还得滑到底才够得到。改成「固定 62% 高的浮层 = 可滚动内容区（`weight(1f)` + `verticalScroll`）+ 滚动区之外的页脚」。真机量过：酸奶/鲜牛奶/睫毛膏/布洛芬四种内容条数不同的物品，浮层顶边一律 y=912、页脚一律 y=2224（改前会漂）
  ...
  - **未做真机验证的一项**：内容超出 62% 高度时"是滚而不是裁"。布局上 `weight(1f)+verticalScroll` 保证可滚，但我造超长备注的探针脚本没跑通（`adb input text` 不吃中文、IME 收起后找不到保存按钮），没拿到证据。手工验法：给任意物品加一段长备注 → 打开详情 → 内容区应能上滑且页脚不动
...
| 详情/设置/撤销 | T11 + 2026-09-26 | **点条目**→详情浮层（全应用没有长按）；给某物品加一段长备注→看内容区能否上滑；删除→Snackbar 撤销；权限卡；动态色；概览计数 | 浮层高度固定、**编辑/删除始终钉在底部不随内容漂**、内容超出时可滚而非被裁；撤销后物品回归且到期日不变；权限状态真实反映；开关即时变色/回落 teal；概览与清点一致 |
