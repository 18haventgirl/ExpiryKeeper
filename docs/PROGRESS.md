...
- [ ] **待用户**：10 项人工验收清单（见下表；恢复 3 项、订阅逾期 1 项、提醒时间 1 项为近期新增）
- [~] M3 同步 / M4 家庭共享 / M2 条码 —— **全部取消**（2026-09-25：没有服务器就不做多人；条码没有可用中文数据源）
- [~] MagicOS 真机杀后台验证、M5 桌面小组件 —— **2026-09-26 用户取消，不再作为待办**
...
- [~] MagicOS 真机杀后台验证 —— **2026-09-26 取消**（用户：不用管这件事）。提醒准时的兜底仍由两层承担：精确闹钟 + WorkManager 每 24h 补扫，只是厂商 ROM 极端杀后台下的表现未做实测
...
- 详情浮层页脚钉住 + 取消恢复撤销条（2026-09-26，用户第二轮验收反馈）
  - **浮层（用户裁决 B，两轮才对齐）**：原先头部、键值、快速操作、最近记录、编辑/删除全塞在同一个 `verticalScroll` 里，按钮跟着内容走、内容一多还得滑到底才够得到。最终形态：**浮层高度写死**（`fillMaxHeight(0.62f)`，每个物品弹出来都一样高，顶边实测 y=912）+ **页脚浮在内容之上**（`Box` 里 `align(BottomCenter)`，不在滚动流的下游），内容从页脚底下穿过，所以页脚自带一层与浮体同色的衬底 + 顶部 20dp 渐隐，滚动内容留出 96dp 底部间隙不被压住。
  - 我中途误读过一次：把"固定高度"理解成"高度自适应、只钉住页脚"并据此改回自然伸缩，用户再次明确才回到写死方案。教训是**"固定住这个元素"不包含也不应顺带改动它的容器**，反过来也一样——容器要不要固定是用户说了算，别替他推断。
  - 衬底颜色用 `surfaceContainerLow`，像素核对过：浮层本体 #f3f3fa、页脚衬底 #f3f3fa 完全一致（不是肉眼看着像，是取点比色），所以不会看成一条灰带；深浅主题都走同一个 token。
  - 一处测量自身的坑记着：`编辑` 也出现在「最近记录」的事件标签里，按文本找按钮必须取 **y 最大**的那个，否则量到记录行（我第一次因此得出"页脚不一致"的假结论）。另一处：截图取样前必须确认浮层真的开着，我第一次那组数（#faf8fe vs #ededf6）是在浮层没打开的画面上取的，得出过"衬底不匹配"的假问题
  - **撤销条取消**（用户裁决）：恢复流程只保留动手前的二次确认，不再事后给「撤销」。连带删掉 `preRestoreSnapshot` / `undoRestore` / `repo.restoreSnapshot` 与 `RestoreDbTest` 里那条撤销断言，并把两处承诺"可以当场撤销"的文案改掉——设置项不能撒谎，这条同样适用于提示条文字。被移出物品仍只打墓碑不物理删，所以真后悔还能靠更早的备份找回
  - 溢出滚动这条**这次有图有真相**：把内容滑到底后截图，头部 emoji 已滚出可视区、五行记录停在页脚上方、页脚纹丝不动（`.debug-ui/audit/footer-scrolled.png`）
- README 与截图（2026-09-26）：新增根目录 `README.md` + `docs/screenshots/`。第一版被用户三处驳回，第二版记在这里，比记第一版长什么样有用：
  - **读者搞错了**：第一版写了「工程约定」「明确的边界（不做什么）」两节——那是咱俩之间的开发约定，不是对外卖点。对外只讲三件事：为什么做、能做什么、怎么实现的。删。
  - **图片渲染不出来**（`image block head no description`）：画廊用了 HTML `<img width>`，用户的渲染器不认。改成标准 `![中文描述](路径)`，alt 文本按"图挂了也说得清这张是什么"来写。
  - **演示数据不合格**：清单里留着 `3213` / `111321` / `22` 三条纯测试用的数字名条目、两件重名「鲜牛奶」，还有一条 `熟食·剩菜` 因为测试时填了 730 天保质期而显示「剩 729 天」——截图会把这种脏数据永久钉在 README 上。先清再拍。
- 截图前的数据整备（全程只用 App UI，没碰过数据库文件）：删掉 5 条测试残留（3213 / 111321 / 22 / 重名鲜牛奶 / 熟食·剩菜），补 4 条有展示价值的条目（鲜肉 剩2天、叶菜 剩5天、坚果 剩90天、猫粮 1 kg 库存低），并把「音乐会员」的下次扣费日设到 9月23日 → 今日屏因此出现了 **续费逾期** 那一类。最终 12 件，覆盖逾期 / 今天 / 临期 / 窗口外 / 低库存 / 续费逾期 / 订阅。
  - 新增条目全靠**常识库模板芯片**点出来，不需要往模拟器里打中文（`adb input text` 打不了中文，这是硬限制）；只有 单位/数量 这类数字字段用 `input text`。扣费日要选过去的日期，`uiautomator` 里日历格子有 `content-desc="Wednesday, September 23, 2026"`，按描述取中心点一下即可。
  - 一个不是 bug 但会看错的现象：`洗衣液` 0.4 瓶 ≤ 低库存线 1 却不报「库存低」，详情显示「节奏正常」——是之前测快速操作时点过「稍后3天」，`snoozedUntilEpochDay` 把它静默到今天之后。引擎没错，但**看截图时以为漏了一类**，于是另加了一条猫粮。
  - 深色那张第一次拍废了：`settings put system night_mode 1` 没生效，而且当时停在设置页（次级页没有「今日」Tab），`wait4("今日")` 找不到就直接拍了张亮色设置页。改用 `cmd uimode night yes` + 先退回根导航再拍。
  - 6 张原图 1080×2424 / 1.3MB，纯 Markdown 没法限宽，会各撑一屏高。用 JBR 的 `ImageIO` 等比缩到 540×1212（共 460KB），原图留在 `.debug-ui/audit/orig/`
  - **第二版又驳回一次**：540 宽一张占满一屏，六张竖着排"篇幅太大、观感不好"，用户要回第一版的两行三列。定版：**纯 Markdown 表格**（`| | | |` + 分隔行当空表头，图像行与说明行交替）配 **360×808** 的图 —— 三张并排 1080px，不再赌渲染器会不会替我们 `max-width:100%` 收缩；HTML `<img width>` 彻底不用，因为它在用户的渲染器里就是显示不出来。六张共 196KB。缩放一律从 `orig/` 的 1080 原图重算，不从已缩过的图二次缩放。
- README 里的每个数字都重新对过代码：75 条 JVM 单测（`test-results` 汇总，0 失败）、40 个主源文件 + 8 个测试文件、9 品类 **44** 条模板（第一版写的 45 是数错了）、minSdk 29、无 `INTERNET` 权限、通知带「稍后 3 天」动作与 group summary
...
| 项 | 来源 | 手测步骤 | 通过标准 |
|---|---|---|---|
| 备份/恢复＝时间点还原 | 2026-09-25 重做 | 导出 → 改一条（改名+改到期日）→ 导入该文件 → 看确认框 → 替换全部 | 确认框先报账（写回 N 条 / 移出 M 件）；替换后那条**回到导出时的样子**（你报的缺陷）；确认框是唯一的保护，没有事后撤销条 |
...
| 添加三步 & emoji | T10 | 「牛奶，开封 3 天」零键盘路径；自定义 🐠 保存；空名/双空规则校验；编辑 M1 旧数据 | ≤15 秒完成；emoji 在今日/清单/详情/通知标题均显示；**保存按钮不置灰**，点了之后才在字段下行内红字提示（A3：首帧不满屏报错）；旧数据不丢字段不崩 |
| 详情/设置/撤销 | T11 + 2026-09-26 | **点条目**→详情浮层（全应用没有长按）；把内容滑到底；删除→Snackbar 撤销；权限卡；动态色；概览计数 | 浮层**高度固定**（每个物品顶边同 y=912）、**编辑/删除浮在内容之上且位置不动**、内容从其下方滑过且页脚衬底与浮体同色（不成灰带）；撤销后物品回归且到期日不变；权限状态真实反映；开关即时变色/回落 teal；概览与清点一致 |
...
### M5 打磨
- [ ] 月度统计（丢弃成本 / 本月处理数，数据源是已有的 `events` 表）
- [ ] 物品照片（需相机/相册权限与存储位置策略，最重的一项）
- [x] ~~桌面小组件 / 图标角标~~ —— **2026-09-26 用户取消，不做**
