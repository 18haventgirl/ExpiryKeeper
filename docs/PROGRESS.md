...
- 详情浮层页脚钉住 + 取消恢复撤销条（2026-09-26，用户第二轮验收反馈）
  - **浮层**：原先头部、键值、快速操作、最近记录、编辑/删除全塞在同一个 `verticalScroll` 里，所以按钮跟着内容走，内容一多还得滑到底才够得到。改成「可滚内容区（`weight(1f, fill = false)` + `verticalScroll`）+ 滚动区之外的页脚」。**浮层高度不写死**——我先做过固定 62%，是多余的：ModalBottomSheet 底边锚在屏幕底、只往上长，所以内容多的物品顶边 y=951、内容少的 1053/1134 各不相同，而页脚三者都是 y=2224（离屏幕底 173px，不被手势条压住）。"高度自适应"与"按钮固定"本来就同时成立，不需要牺牲前者
  - 一处测量自身的坑记着：`编辑` 也出现在「最近记录」的事件标签里，按文本找按钮必须取 **y 最大**的那个，否则量到记录行（我第一次就得出"页脚不一致"的假结论）
  ...
  - 未做真机验证的一项：内容超出可用高度时"是滚而不是裁"。现有物品里最长的内容只到 y=2096，还没顶到页脚上沿（2198），这条路径在日常数据下走不到；布局上 `weight(1f, fill = false) + verticalScroll` 保证溢出时可滚
...
| 详情/设置/撤销 | T11 + 2026-09-26 | **点条目**→详情浮层（全应用没有长按）；删除→Snackbar 撤销；权限卡；动态色；概览计数 | 浮层高度随内容伸缩、但**编辑/删除始终钉在浮层下沿不随内容漂**（实测三个物品页脚同为 y=2224）；撤销后物品回归且到期日不变；权限状态真实反映；开关即时变色/回落 teal；概览与清点一致 |
