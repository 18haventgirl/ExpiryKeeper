...
- README 与截图（2026-09-26）：新增根目录 `README.md` + `docs/screenshots/`（6 张真机截图：今日 / 清单 / 添加含模板条 / 详情浮层 / 设置含每日提醒 / 暗色今日，共 1.3MB）。截图由脚本在 emulator-5554 上逐屏取证并**逐张看过**（不是"应该长这样"）；拍摄前确认过清单里 12 件全是通用测试条目，无个人信息，因此没有动用户数据。README 里的每条能力、每个数字（75 单测 / 48 个 .kt / 9 品类 45 模板 / minSdk 29）都对着代码与构建产物核过
