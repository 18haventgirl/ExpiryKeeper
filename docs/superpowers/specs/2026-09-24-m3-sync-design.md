...
日期：2026-09-24　状态：**已作废（2026-09-25）**——用户决定「没有服务器就不做多人功能」，M3 与 M4 一并撤下。文件保留作决策与分析和解空间记录（§3 全量 vs 增量、§6.2 收敛性、§7 数据安全不变量这些推导本身与是否实现无关），**不要照此实现**。
上游文档：`docs/superpowers/specs/2026-09-22-expiry-keeper-design.md` §8（同步一期设想）
进度记录：`docs/PROGRESS.md`（M3/M4 取消记录与决策日志）
